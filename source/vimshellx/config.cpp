...
int SelectMyIcon ( int iWantSz, bool &fCenterIcon, bool &fHQBScaling )
{
  ...
  if ( iWantSz < 15 ) { return IDB_MENU_BITMAP_13; }
  if ( iWantSz < 18 ) { return IDB_MENU_BITMAP_15; }
...
// ****************************************************************************
//
// Title string format is ... "autoexec.bat = (C:\) - GVIM"
// Title string format is ... "[No Name] = - GVIM"
// Title string format is ... "[No Name] - GVIM"
  ...
  wchar_t * pString, wchar_t * &pHead, wchar_t * &pFile, wchar_t * &pTail )
{
  ...
  size_t iString = wcslen( pTail );

  pTail = wcschr( pString, L':' );
  if ( NULL == pTail-- )
  {
    pTail = wcschr( pString, L'\\' );
  }
  ...
  if ( pTail )
  {
    while ( pTail > pHead )
    {
      if ( *--pTail == L'(' )
      {
        size_t iTail = wcslen( --pTail );
        wmemcpy( pTail, L" - GVIM", ( iTail < (7+1) ? iTail : (7+1) ) );
        return;
      }
    }
  }
